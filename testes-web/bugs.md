...
### Impacto:
Usuário pode não compreender que ambos os campos são obrigatórios, gerando tentativas incorretas de login e frustração no uso da aplicação.  

### Evidência:
![mensagem de erro de login](evidencias/bug-001-login.png)

### Observação:
A validação ocorre apenas para o campo de usuário, sem validar simultaneamente o campo de senha quando ambos estão vazios.
...
### Observação:
O problema ocorre apenas na listagem de produtos.  
Ao acessar o detalhe do item, a imagem correta é exibida, indicando inconsistência na renderização da listagem.

---

## BUG-003 – Ausência de funcionalidade para seleção de quantidade do produto

**Relacionado ao caso de teste:** CT-05 – identificação inicial, com validação complementar nos demais perfis  
**Usuário impactado:** Todos os usuários  
...
**Severidade:** Baixa  
**Prioridade:** Baixa  
**Tipo de defeito:** UX / Possível melhoria  

### Passos para reprodução:
1. Acessar a página de login  
2. Inserir um usuário válido (`standard_user` ou `problem_user`)  
3. Inserir a senha `secret_sauce`  
4. Clicar em "Login"  
5. Acessar a lista de produtos  
...
### Impacto:
Usuário fica impossibilitado de adquirir múltiplas unidades do mesmo produto, limitando a funcionalidade do carrinho e podendo impactar negativamente a experiência de compra e a conversão de vendas.  
